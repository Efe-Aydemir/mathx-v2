...
def main():
    ...
    # Note: YAML may parse scientific notation (e.g. 2e-4) as string, so we cast explicitly
    epochs = int(args.epochs or train_cfg.get("num_train_epochs", 3))
    lr = float(args.lr or train_cfg.get("learning_rate", 2e-4))
    ...
    lr_scheduler = str(train_cfg.get("lr_scheduler_type", "linear"))
    seed = int(train_cfg.get("seed", 42))
    output_dir = args.output_dir or str(train_cfg.get("output_dir", "../outputs"))
